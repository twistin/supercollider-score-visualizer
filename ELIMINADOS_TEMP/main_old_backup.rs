...
fn update(app: &App, model: &mut Model, _update: Update) {
    ...
    while let Ok(msg) = model.osc_receiver.try_recv() {
        println!("📡 MENSAJE OSC RECIBIDO: {} con {} args: {:?}", msg.addr, msg.args.as_ref().map_or(0, |args| args.len()), msg.args);
...
fn parse_realtime_audio(msg: &osc::Message, current_time: f64) -> Option<AudioAnalysis> {
    ...
    Some(AudioAnalysis {
        ...
        harmonicity: args.get(5).map_or(0.5, |arg| get_float(arg).unwrap_or(0.5)),
        noisiness: args.get(6).map_or(0.5, |arg| get_float(arg).unwrap_or(0.5)),
