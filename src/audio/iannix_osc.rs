...
impl IanniXOscProcessor {
    ...
    fn process_continuous(&mut self, msg: &OscMessage, timestamp: f64) -> Result<bool, String> {
        if msg.args.len() >= 5 {
            ...
            if let (Some(freq), Some(amp), Some(centroid), Some(brightness)) =
                    (get_float(&msg.args, 1), get_float(&msg.args, 2), get_float(&msg.args, 3), get_float(&msg.args, 4)) {
                ...
                if msg.args.len() >= 7 {
                    if let (Some(freq_vel), Some(amp_vel)) = 
                           (get_float(&msg.args, 5), get_float(&msg.args, 6)) {
    ...
    fn process_trigger(&mut self, msg: &OscMessage, timestamp: f64) -> Result<bool, String> {
        if msg.args.len() >= 4 {
            if let (Some(OscType::String(event_type)), 
                    Some(intensity), 
                    Some(position)) = 
                    (msg.args.get(1), get_float(&msg.args, 2), get_float(&msg.args, 3)) {
    ...
    fn process_control(&mut self, msg: &OscMessage, _timestamp: f64) -> Result<bool, String> {
        if msg.args.len() >= 3 {
            if let (Some(OscType::Int(control_id)), 
                    Some(value)) = 
                    (msg.args.get(1), get_float(&msg.args, 2)) {
    ...
    pub fn get_data_mut(&mut self) -> &mut IanniXOscData {
        &mut self.data
    }
...
mod tests {
    ...
    fn test_control_message() {
        ...
        assert!(processor.process_message(&msg).is_ok());
        assert_eq!(processor.get_data().controls.get(&42), Some(&0.9));
    }
}
