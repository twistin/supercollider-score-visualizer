...
impl OscHandler {
    ...
    fn parse_event_message(args: &[osc::Type]) -> anyhow::Result<MusicalEvent> {
        ...
        match event_type.as_str() {
            "point" => parse_point_event(freq, amp, dur, visual_params),
            "gliss" | "glissando" => parse_glissando_event(freq, amp, dur, args, visual_params),
            "cluster" => parse_cluster_event(freq, amp, dur, args, visual_params),
            "noise" => parse_noise_event(freq, amp, dur, args, visual_params),
            _ => Err(VisualizerError::event_parsing(
...
fn parse_point_event(freq: f32, amp: f32, dur: f32, visual_params: Option<VisualParams>) -> anyhow::Result<MusicalEvent> {
...
fn parse_glissando_event(freq: f32, amp: f32, dur: f32, args: &[osc::Type], visual_params: Option<VisualParams>) -> anyhow::Result<MusicalEvent> {
    let end_freq = match args.get(4) {
        Some(osc::Type::Float(f)) => *f,
        Some(osc::Type::Int(i)) => *i as f32,
        _ => return Err(VisualizerError::event_parsing(
            "glissando".to_string(),
...
fn parse_cluster_event(freq: f32, amp: f32, dur: f32, args: &[osc::Type], visual_params: Option<VisualParams>) -> anyhow::Result<MusicalEvent> {
    let bandwidth = match args.get(4) {
        Some(osc::Type::Float(f)) => *f,
        Some(osc::Type::Int(i)) => *i as f32,
        _ => return Err(VisualizerError::event_parsing(
            "cluster".to_string(),
...
fn parse_noise_event(freq: f32, amp: f32, dur: f32, args: &[osc::Type], visual_params: Option<VisualParams>) -> anyhow::Result<MusicalEvent> {
    let bandwidth = match args.get(4) {
        Some(osc::Type::Float(f)) => *f,
        Some(osc::Type::Int(i)) => *i as f32,
        _ => return Err(VisualizerError::event_parsing(
